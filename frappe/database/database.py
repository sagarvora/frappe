	...
	def bulk_update(
		...
		*,
		...
		for __ in range(0, total_docs, chunk_size):
			...
			self.build_query_and_update_in_bulk(doctype, doc_chunk, debug=debug)

		if update_modified:
			modified_dict = self._get_update_dict(
				{}, None, modified=modified, modified_by=modified_by, update_modified=update_modified
			)
			frappe.db.set_value(doctype, doc_updates.keys(), modified_dict, update_modified=False)

	@staticmethod
	def build_query_and_update_in_bulk(doctype: str, doc_updates: dict, *, debug=False):
		"""
		:param doctype: DocType to update
		:param doc_updates: Dictionary of key (docname) and values to update
		:param debug: Print the query in the developer / js console.
